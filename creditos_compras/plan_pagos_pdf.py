"""
Genera el PDF del Plan de Pagos / Estado de Cuenta de una compra a
crédito. Documento separado de purchasing/purchase_pdf.py (esa es la
compra fija, con el detalle de productos); este refleja el estado de
las cuotas y pagos AL MOMENTO de generarlo, no un documento fijo.
Mismo estilo visual que creditos_ventas/plan_pagos_pdf.py.
"""
...
def generar_pdf_plan_pagos_compra(purchase):
    """Devuelve los bytes del PDF del plan de pagos de la compra `purchase`."""
    buffer = io.BytesIO()
    ...
    elements.append(Paragraph(f'<b>Plan de Pagos - Compra #{purchase.id}</b>', styles['Title']))
    ...
    elements.append(Paragraph(f'Tipo de pago: {purchase.get_tipo_pago_display()}', styles['Normal']))
    elements.append(Paragraph(f'Estado de la compra: {purchase.get_estado_display()}', styles['Normal']))
    elements.append(Paragraph(f'Saldo pendiente actual: {money(purchase.saldo)}', styles['Normal']))
    ...
    for cuota in purchase.cuotas.all().order_by('numero'):
        cuota_rows.append([
            str(cuota.numero),

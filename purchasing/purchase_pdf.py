...
def generar_pdf_compra(purchase):
    ...
    encabezado = Table(
        [[Paragraph('Sistema de Ventas', negocio_style), Paragraph('COMPRA', titulo_doc_style)]],
        colWidths=[9 * cm, 9 * cm],
    )
    ...
    # --- Bloque de datos: proveedor a la izquierda, fecha/compra/forma de pago a la derecha ---
    if purchase.tipo_pago == 'CREDITO':
        cuotas_count = purchase.cuotas.count()
        forma_pago = f'CRÉDITO ({cuotas_count} cuota{"s" if cuotas_count != 1 else ""})'
    else:
        forma_pago = purchase.get_tipo_pago_display()

    datos_izq = [
        Paragraph('Comprar a:', label_style),
        Paragraph(purchase.supplier.name, normal_style),
    ]
    if purchase.supplier.email:
        datos_izq.append(Paragraph(purchase.supplier.email, normal_style))
    datos_izq.append(Paragraph(f'N° Documento: {purchase.document_number}', normal_style))

    datos_der = [
        Paragraph(f'<b>FECHA:</b> {purchase.purchase_date.strftime("%d/%m/%Y %H:%M")}', datos_der_style),
        Paragraph(f'<b>COMPRA N°:</b> {purchase.id}', datos_der_style),
    ...
    for d in purchase.details.all():
        rows.append([d.product.name, str(d.quantity), _money(d.unit_cost), _money(d.subtotal)])
    ...
    # --- Pie de página ---
    elements.append(Paragraph('Gracias por su pronta entrega', footer_style))
    if purchase.tipo_pago == 'CREDITO':
        elements.append(Paragraph(
            'Esta compra es a crédito. Consulta el plan de pagos para ver el detalle de las cuotas.',
            footer_nota_style,
        ))
    elements.append(Paragraph(
